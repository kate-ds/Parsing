...
class GbParse:
    ...
    def pag_parse(self, url, soup):
        ...
        self.create_parse_tasks(url, soup.find('div', attrs={'class': 'post-items-wrapper'}).find_all('a', attrs={'class': 'post-item__title'}), self.post_parse)
    ...
    def post_parse(self, url, soup):
        ...
        tags_a = soup.find('article', attrs={'class': 'blogbost__article-wrapper'}).find_all('a', attrs={'class': 'small'})
        tags = [{'url': urljoin(url, tag.get('href')),
                 'name': tag.text} for tag in tags_a]
        return {
            'post_data': post_data,
            'author': author,
            'tags': tags
